"""Research routes (Plan 5, D92): read-only views over what the scan already recorded.

Every route here is a GET. There is deliberately no endpoint that starts a scan or promotes a candidate: a scan
reads many sessions of stored bars and belongs to the worker, and promotion into a paper signal is an explicit
decision that goes through the existing `POST /signals` intake with its own validation and idempotency. Research
observations stay isolated until someone decides otherwise, and that decision is not an HTTP side effect of
looking at a list.
"""

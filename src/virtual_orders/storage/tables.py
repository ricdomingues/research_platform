...
pattern_detections = Table(
    ...
    # A change-detection key, NOT the set of bars this detection read. It covers the whole window that could
    # have influenced a reading at this bucket: the pattern's candles plus the prior-trend lookback behind
    # them, because `evidence_hash` carries that prior trend. Two detections of one bucket sharing this value
    # were derived from data that did not change between them; a difference means something upstream did.
    # Nullable: rows written before migration 0007 have no such identity and were not given a fabricated one.
...
setup_candidates = Table(
    ...
    # Copied from the detection this candidate was built on, and NOT a valid change-detection key for the
    # candidate itself. A candidate's features come from `compute_series` and `structure_at` over the WHOLE
    # candle series, so a correction anywhere earlier in the session changes them while this value stays put.
    # Whatever retires a stale candidate must not reuse the detection guard's shape; follow the parent
    # detection instead. Nullable, for the same reason as on `pattern_detections`.
    Column("input_content_hash", Text, nullable=True),
    _ts("data_as_of"), _ts("created_at"),
)
